...
# WebAppSecurityTester class definition

@app.route("/", methods=["GET", "POST"])
def index():
    if request.method == "POST":
        base_url = request.json.get("base_url")  # Get base_url from JSON payload
        if not base_url:  # Validate the base_url
            return jsonify({"error": "Base URL is required"}), 400

        try:
            params = json.loads(request.json.get("params", "{}"))  # Safely parse parameters
        except json.JSONDecodeError:
            return jsonify({"error": "Invalid JSON in Parameters"}), 400

        tester = WebAppSecurityTester(base_url)
        tester.run_tests(params)
        return jsonify(tester.vulnerabilities)
